(function ($) {
  Drupal.behaviors.udlamenuimageUpload = {
    attach: function (context, settings) {
      $('.form-item-udlamenuimage .form-managed-file').ajaxComplete(function (event, request, settings) {
        ...
        if (submitButton.length != 0 && previewImage.length != 0) {
          switch (submitButton[0].value) {
            ...
            case "Upload":
              $($(previewImage[0])).attr("src", Drupal.settings.udlamenuimageBlank).attr({'width': 75, 'height': 75});
              break;
  ...
  $(document).ready(function () {
    $('.form-item-udlamenuimage .form-managed-file span a').click(function (event) {event.preventDefault();});
  });
}) (jQuery);
