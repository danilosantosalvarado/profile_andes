         ...
         if($push_id_agent == '0' && $push_department <> '') 
              {
                 $queryu = "SELECT  id, department, device_id , device ".
                           ...
                           " WHERE (UNIX_TIMESTAMP(NOW()) - UNIX_TIMESTAMP(`refresh`)) < '$connection_timeout_mobile' and device_id <>'' and ".
                       ...
                       if (is_array($rowu))
                         {
                           ...
                           if ($push_device=="IOS") {
						     $call_URL = $push_api_path . "notify_ios.php?token=".$push_user_device. "&message="  . "$chat_request" .  "$push_username";
						   } else
							   
						  if ($push_device=="Android") {
						     $call_URL = $push_api_path . "notify_android.php?token=".$push_user_device. "&message="  . "$chat_request" .  "$push_username";
						    } 
